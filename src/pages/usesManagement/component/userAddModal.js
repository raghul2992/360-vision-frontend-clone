...
import SendInvitationConfirm from './SendInvitationConfirm'
...
const AddUserModal = ({ isOpen, onClose, tenantId }) => {
  ...
  const { isLoading: isCreating, isInviting } = useSelector(
    state => state.users
  )
  ...
  const [formData, setFormData] = useState({
    ...
    email: '',
    role: 'admin',
    meta: { assign_locations: [] }, // This will now store IDs
    password: ''
  })

  const currentUserRole = localStorage.getItem('user_role') // 'admin', 'operator', or 'viewer'

  // 2. Define which roles are available based on who is logged in
  ...
  // UPDATED: Logic to handle ID instead of Name
  const handleLocationToggle = locationId => {
    if (!locationId || locationId === 'default') return

    // Ensure ID is handled as a string for consistent comparison
  ...
  const handleSubmit = async () => {
    ...
    try {
      // The payload meta.assign_locations already contains the IDs from state
      await dispatch(addNewUser({ tenant_id: tenantId, ...formData })).unwrap()
      toast.success(t('userManagement.addModal.successTitle') || 'User Added!')
  ...
  const handleSendInvitation = async () => {
    try {
      ...
      toast.error(err || t('userManagement.errors.inviteFailed'))
    }
  }
    ...
    <div className='fixed inset-0 z-[90] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4'>
      <div className='bg-[#2c2d3a] w-full max-w-3xl rounded-2xl border border-gray-700 max-h-[90vh] flex flex-col shadow-2xl overflow-hidden'>
        {!showInviteConfirm ? (
              ...
              <div className='space-y-4'>
                <div>
                  ...
                  <input
                    type='text'
                    placeholder={t(
                      'userManagement.addModal.fullNamePlaceholder'
                    )}
                ...
                <div>
                  <label className='text-sm font-medium text-gray-300 block mb-2'>
                    {t('userManagement.addModal.password')}
                  </label>
                ...
                <div className='space-y-5 pt-4 border-t border-gray-700/50 animate-in fade-in duration-300'>
                  <div className='w-full h-40 rounded-xl overflow-hidden border border-gray-700 bg-[#1c1c24]'>
                    {isLoaded && (
                      ...
                      >
                        {locations.map(l => {
                          // Check selection status using ID
                          const isSelected =
                            formData.meta.assign_locations.includes(
                              l.id.toString()
                            )
                          return (
                            <Marker
                    ...
                    <div className='relative'>
                      ...
                      >
                        ...
                        {locations.map(l => (
                          <option
                            key={l.id}
                            value={l.id} // Passing ID as value
                  ...
                  {/* CHIPS: UI shows Name, Logic uses ID */}
                    ...
                    <div className='flex flex-wrap gap-2'>
                      {formData.meta.assign_locations.length > 0 ? (
                        formData.meta.assign_locations.map(locId => {
                          // FIND THE NAME FROM THE LOCATIONS ARRAY USING THE ID
          ...
          <div className='flex-1 flex flex-col items-center justify-center bg-[#2c2d3a] overflow-hidden'>
            <SendInvitationConfirm
              title={t('userManagement.addModal.successTitle') || 'Success!'}
